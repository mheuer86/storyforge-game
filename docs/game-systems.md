...
| **Origin counters** | Per-origin moral pressure counters. Every moral-category decision auto-ticks UP; enforcement language reverses to DOWN. At threshold 3: "rising" warning. At threshold 5: permanent identity shift (species mutates to shifted label, behavioral directive changes). Non-moral decisions with moral weight use explicit `origin_event`. |
| **Identity shifts** | One-way gate at origin counter threshold 5. Mutates `character.species` to a shifted label (e.g. Synod-Raised → Heretic). Shifted species have their own behavioral directives. The shift changes how Claude narrates interiority and how NPCs react. Hidden from character creation via `hidden: true` on the Species interface. |
