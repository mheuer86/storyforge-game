...
| **Identity shifts** | Permanent species mutation at counter threshold (10 for standard, 0 for Minor House low-end). Mutates `character.species` to a shifted label (e.g. Synod-Raised → Heretic). Shifted species have their own behavioral directives. Hidden from character creation via `hidden: true` on the Species interface. |
