...
const ROLL_REMINDER_FAILURE = ' This is a FAILURE. The character does NOT learn what they were trying to learn. Do NOT reveal the truth. For Insight/Perception/Investigation failures, choose the outcome that best fits the scene: (A) RED HERRING — the character reads the situation wrong and believes something false. Add a clue with a confident title presenting the wrong belief as fact, set is_red_herring: true. Best when the wrong conclusion drives the story forward. (B) NO READ — the character simply cannot get a read. No clue created. "Her expression is unreadable." Best when the failure IS the information (the NPC is guarded, the scene is opaque). (C) AMBIGUOUS SIGNAL — the character notices something but cannot interpret it. No clue, but plant a narrative seed. "She shifts when Torsten speaks, but you cannot tell why." Best for building tension. Never reveal the truth. Never hedge with doubt that points toward the right answer. Never label a clue as "false" or "misleading" — the character believes what they believe. Never break the fourth wall to explain game mechanics. For Persuasion/other failures: the approach fails and something gets worse (disposition drop, closed door, time lost, complication). Then call commit_turn with ONLY the state changes caused by this failure. Do NOT re-send inventory_use, credits_delta, or other changes from the pre-roll commit_turn — those were already applied.'
