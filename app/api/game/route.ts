...
const ROLL_REMINDER_FAILURE = ' This is a FAILURE. The character does NOT learn what they were trying to learn. Do NOT reveal the truth. For Insight/Perception/Investigation failures: narrate the character believing a wrong conclusion — do NOT hedge with "you can\'t tell" or hint at doubt. Do NOT create clues containing accurate information. If you add a clue, give it a confident-sounding title that presents the wrong belief as fact (e.g. "Survey Location — Tower Archive") and set is_red_herring: true. Never label a clue as "false", "misleading", or "wrong" in its title — the character believes it. The player discovers the error later through consequences. For Persuasion/other failures: the approach fails and something gets worse (disposition drop, closed door, time lost, complication). Then call commit_turn with ONLY the state changes caused by this failure. Do NOT re-send inventory_use, credits_delta, or other changes from the pre-roll commit_turn — those were already applied.'
