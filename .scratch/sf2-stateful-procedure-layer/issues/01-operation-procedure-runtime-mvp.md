...
Verified built on top of the shared procedure kernel.

- Added optional `campaign.procedures`, `world.operationRuntimeId`, and `world.activeProcedureIds` so live operation runtime state can coexist with the singleton intended `operationPlan`.
- Save normalization initializes the procedure registry and migrates legacy shallow `world.operation` into `campaign.procedures.procedure_operation_current` without dropping the old operation plan.
- Archivist create/update/transition can now create and mutate `procedure` records; operation runtime patches support objectives, procedure facts, constraints, affordances, complications, support contributions, assessments, abort conditions, signals, links, status, and semantic phase.
- Narrator retrieval now emits bounded operation procedure packets and the V2 play HUD renders active operation phase/objectives, constraints, facts, abort conditions, and ready signals.
- Review correction: operation packets now require both `status: "ready"` and `consumed: false` before surfacing coordination signals, so legacy inconsistent signal data cannot reappear as actionable.
