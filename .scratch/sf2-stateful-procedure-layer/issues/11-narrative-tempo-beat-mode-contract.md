...
Verified built as a deterministic per-turn beat-mode contract.

- Added `lib/sf2/beat-mode.ts` with the closed beat-mode enum, per-mode word/roll/action/close/scope guidance, deterministic derivation from player input, active procedure state, combat/exploration state, operation phase/status, current interlocutors, and recent resolution state.
- Mechanics packets and Narrator per-turn context now carry the selected beat mode and render a private beat-mode block for the Narrator.
- Meta turns suppress pacing diagnostics and rendered pacing advisory text so rules negotiation does not trigger forward pressure.
- Planning/access/debrief/etc. guidance relaxes scope and roll pressure where the ticket calls for it without changing chapter cap behavior.
