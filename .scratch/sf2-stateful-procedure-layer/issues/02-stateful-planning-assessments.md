...
Verified built as a bounded helper and replay assertion layer.

- Added `lib/sf2/procedure-planning.ts` with planning support normalization, assessment normalization, and roll-support derivation for advantage/DC shift/adverse support.
- Runner now asserts `expected.procedurePlanningSupport`, so planning helper fixtures are executable rather than documentation-only.
- Verified with `npm run build` and `npm run sf2:replay -- fixtures/sf2/replay/procedure-planning-support-assessment.json`.
