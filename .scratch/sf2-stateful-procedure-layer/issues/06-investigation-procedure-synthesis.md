...
Verified built as an investigation synthesis support slice.

- Added `lib/sf2/procedure-investigation.ts` with auto-activation when an active investigation procedure exists or multiple clues accumulate, bounded synthesis packets, hypotheses, contradictions, open questions, next leads, and a clue-vs-procedure-fact discriminator.
- Runner now asserts `expected.procedureInvestigation`.
- Verified with `npm run build` and `npm run sf2:replay -- fixtures/sf2/replay/procedure-investigation-synthesis.json`.
