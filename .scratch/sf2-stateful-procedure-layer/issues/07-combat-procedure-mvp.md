...
Verified built as a combat procedure packet slice.

- Added `lib/sf2/procedure-combat.ts` with closed combat exit conditions and a bounded packet that projects encounter name, round, initiative, active enemies, player position, hazards, objectives, exit conditions, and parent operation/access procedure ids.
- Mechanics packets now attach combat runtime data while preserving active operation constraints alongside combat-first beat mode.
- Verified with `npm run build` and `npm run sf2:replay -- fixtures/sf2/replay/procedure-combat-packet-coexists-operation.json`.
