...
Verified built as a deterministic Arc Author pass-bar evaluator.

- Added `lib/sf2/arc-author/procedure-eval.ts` to evaluate durable forces, stance axes, endgames, pressure-job chapter functions, alternate stance validity, non-operation seed safety, and deadline/surveillance/military/trust pressure coverage.
- Added a recorded finding at `.scratch/sf2-stateful-procedure-layer/artifacts/arc-author-eval-01.md`.
- Runner now asserts `expected.arcAuthorProcedureEval`.
- Verified with `npm run build` and `npm run sf2:replay -- fixtures/sf2/replay/arc-author-procedure-eval-passbar.json`.
