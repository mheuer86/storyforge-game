      ...
      creates: {
        ...
        items: {
          ...
          properties: {
            kind: {
              type: 'string',
              enum: ['npc', 'faction', 'thread', 'decision', 'promise', 'obligation', 'clue', 'arc', 'location', 'temporal_anchor', 'document', 'procedure'],
            },
            payload: {
              type: 'object',
              description:
                'Flat fields for the entity. Shape depends on kind. All retrieval_cue values must be ≤12 words: short search handles, not sentence summaries. NPCs: {name, affiliation, role, pronoun: "she/her"|"he/him"|"they/them"|"other", age, profile_facts: string[], voice_note, voice_register, retrieval_cue, key_facts: string[], supersedes_id?: string}. profile_facts are durable dossier/profile comparison baselines such as "official profile says younger"; include only load-bearing facts. The optional supersedes_id field directs an anonymous→named merge: when the prose names a previously-anonymous on-stage NPC (e.g. "the unknown young man" turns out to be "Sev\'s brother"), set supersedes_id to that existing anonymous NPC\'s id. Apply-patch will rename + enrich the existing entity rather than creating a parallel one. Threads: {title, owner: {kind: "npc"|"faction", name_or_id: string}, tension: number 0-10, resolution_criteria, failure_mode, retrieval_cue, resolution_mode?: "investigation"|"pressure", resolution_gates?: [{id,label,condition,required?: boolean}], successor_to_thread_id?: thread id/title, arc_id?: existing arc id/title}. Set resolution_mode:"investigation" only for threads whose resolution depends on discovering, proving, or connecting evidence; ordinary pressure/obligation threads are pressure. Use resolution_gates only when successful resolution requires distinct state facts; code blocks successful resolution until required gates are satisfied. Use successor_to_thread_id when a resolved thread opens a follow-up question; code automatically anchors the successor to the same arc as its predecessor. Decisions: {summary, anchored_to: [thread_title_or_id, ...]}. Promises: {obligation, owner: {kind, name_or_id}, anchored_to: [thread_title_or_id, ...]}. Clues: {content, evidence_question, evidence_kind: "document"|"testimony"|"trace"|"contradiction"|"diagnostic"|"circumstantial", anchored_to: [investigation_thread_title_or_id, ...] (may be empty only when the evidence is real but the PC has not connected it yet)}. Clues are investigation evidence, not a bucket for body language, routine presence, atmosphere, or small operational state. Locations: {name, description, atmospheric_conditions: string[], locked?: boolean}. Temporal anchors: {title, kind: deadline|timestamp|duration|sequence|recurrence, label, anchor_text, anchored_to: [entity_id_or_title, ...], retrieval_cue}. Deadline temporal anchors anchored to threads are the canonical timeline fact for the thread timer; do not duplicate the same deadline as a separate fact. Arcs: {title, thread_ids: [...], spans_chapters: number, stakes_definition: string}. Documents: create only for named artifacts whose terms or attribution matter in future play: {type: "authorization"|"directive"|"communication"|"record"|"petition"|"notation", kind_label: string (genre flavor noun, e.g. "writ", "transfer order", "ship manifest"), title, authorizes (one-line: what it permits/commands/attests/requests), original_summary (canonical terms at issuance — locked, never overwritten by updates), filed_by (npc/faction id-or-name; who originated/registered it), signed_by (npc/faction id-or-name; who authorized it — distinct from filer), additional_parties: [{role: "counter-signer"|"witness"|"custodian"|..., entity_id: string}], subject_entity_ids: [npc/faction ids; whom the document concerns — REQUIRED, must be ≥1. Use canonical IDs from the cast roster (e.g. \\`npc_4\\`, \\`npc_kess_elder\\`). Do NOT synthesize role-descriptive IDs like \\`npc_fled_resonant\\` when an existing NPC matches that role — scan the cast roster and reference their canonical id. Synthesizing role-descriptive IDs causes the same person to be treated as multiple entities in downstream prose.], anchored_to: [thread ids; may be empty for floating], access_level?: "public"|"sealed"|"classified", retrieval_cue}. Procedures: {kind:"operation"|"access"|"exploration"|"investigation"|"combat"|"montage_task", label, status:"active"|"paused"|"resolved"|"failed"|"abandoned", phase:"orientation"|"commitment"|"preparation"|"engagement"|"egress"|"reckoning"|"aftermath", objective, stakes, facts:[{id,text,confidence,sourceRefs,lastUpdatedTurn}], constraints:[{id,label,kind,status,current?,max?,clearsWhen?,linkedRefs}], affordances:[...], complications:[...], contributions:[...], assessments:[...], abort_conditions:[...], signals:[...], linked_refs:[...]}. Procedure facts are not investigation clues unless they answer an investigation question.',
              properties: {
                name: { type: 'string' },
                pronoun: { type: 'string', enum: ['she/her', 'he/him', 'they/them', 'other'] },
                age: { type: 'string' },
                profile_facts: { type: 'array', items: { type: 'string' }, maxItems: 3 },
                ...
                kind: { type: 'string', enum: ['deadline', 'timestamp', 'duration', 'sequence', 'recurrence', 'operation', 'access', 'exploration', 'investigation', 'combat', 'montage_task'] },
                resolution_mode: { type: 'string', enum: ['investigation', 'pressure'] },
                evidence_kind: { type: 'string', enum: ['document', 'testimony', 'trace', 'contradiction', 'diagnostic', 'circumstantial'] },
                evidence_question: { type: 'string' },
              },
